...
# --- (2) Thêm tham số time_limit vào hàm solve ---
def solve(input_content=None, time_limit=0.95):
    start_time = time.time() # <--- (3) Ghi nhận thời gian bắt đầu
    ...
    # --- BƯỚC 1: TẠO LỜI GIẢI KHỞI TẠO BẰNG GREEDY ---
    ...
    # Greedy Construction
    for task in tasks:
        # (4) Kiểm tra thời gian trong vòng lặp chính (đề phòng dữ liệu quá lớn)
    ...
    # --- BƯỚC 2: LOCAL SEARCH (Optional) ---
    MAX_ITER = 1000
    for _ in range(MAX_ITER):
        # (5) Kiểm tra thời gian trong vòng lặp cải thiện
        if time.time() - start_time > time_limit: 
            break
            
        if not unassigned_tasks: break 
        # Logic Local Search đơn giản (bị break sớm như code gốc)
        break 
